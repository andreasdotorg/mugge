...
| US-070 | IMPLEMENT | 0/7 | **Pivot to GitHub-hosted runners with `cachix/install-nix-action` per owner directive.** Workflow YAML (`6db6f28`) needs rewrite for new AC. **worker-ci assigned 2026-03-23.** |
...
| US-075 | **IMPLEMENT** | **TBD** | **Merged to main** (`9d31713`, 2026-03-23). Production-replica local PW test env: virtual USBStreamer/ADA8200 nodes with production names, filter-chain convolver with Dirac impulses + 4 gain nodes, GM measurement mode, managed-mode signal-gen + pcm-bridge. Rule 13: Architect APPROVED, QE APPROVED. Remaining: AC #4 (automated verification script), AC #5 (`nix run .#test-integration` target), AC #6 (reusable library API), AC #7 (Rust test consolidation). |
...
- **US-070** (Phase: **IMPLEMENT** — committed `6db6f28`): CI Setup. Workflow YAML + runner setup script + `test-everything` flake target all committed. Enables branch-based parallel work (resolves L-039). DoD TBD — needs PO acceptance criteria. **Owner provided `cachix/install-nix-action` reference** — may eliminate need for self-hosted runner (GitHub-hosted runners + Nix install action). Simplifies setup significantly.
