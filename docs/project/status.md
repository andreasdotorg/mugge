...
- **GraphManager deployment** (in-progress — 2026-03-20): GM-7 DONE (`eaf28ae`). GM systemd service committed (`f9473b0`). GM Pi deployment ready — pending owner go-ahead for reboot (USBStreamer transient risk). US-052 D-040 adaptation in progress (worker assigned).
...
- **US-052** (ACTIVE — resumed per owner directive 2026-03-20): RT Signal Generator. D-040 adaptation in progress. SG-12 blocker resolved by D-040. Worker assigned. Was IMPLEMENT 3/6 (6,183 lines, 193 tests).
