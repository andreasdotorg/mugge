...
- **US-051** (selected, in implementation): Persistent system status bar with channel meters. SB-1 DONE (HTML/CSS scaffold). Next: SB-2 (API) then SB-3 (data wiring, critical path). Critical path: ~~SB-1~~ → SB-3 → SB-6 → SB-7.
