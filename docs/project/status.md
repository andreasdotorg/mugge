...
| US-004 | REVIEW | 3/4 | in-review (assumption register written with A1-A26, cross-references documented, CLAUDE.md updated. Accuracy corrections committed `11d0292`. **Gap:** AC mentions A27 but register only has A1-A26.) |
...
| US-052 | VERIFY | 4/6 | **VERIFY PASS** (S-001 2026-03-21). Signal-gen running on Pi with workarounds for F-034 (clap negative value) and F-035 (seccomp SIGSYS). Responds to RPC. Repo service file needs fixes before clean deployment. |
...
| US-059 | TEST | **14/14** | **TEST phase (advanced 2026-03-21).** QE confirmed existing test coverage satisfies formal test plan requirement (168 regression tests S-1-S-4 + I-1 green, DoD #8 stability + #9 latency with hardware evidence, QE sign-off #13 unconditional). All 14 DoD items satisfied. GM tasks: 14/15 (GM-8 CamillaDSP removal tracked separately). HEAD: `5fbd5f7`. Follow-ups: F-033, I-1 CI wiring, spectral verification (AC 3141), D-042 lifting. **Next phase: DEPLOY.** |
| US-060 | DEPLOY | 1/7 | **BLOCKED** (S-001 2026-03-21). Files in git checkout but NOT deployed to production `~/web-ui/` path on Pi. Production still has old CamillaDSP collectors. Needs redeployment to production path. Blocks US-050, US-051. |
| US-061 | DEPLOY | 0/8 | **BLOCKED** (S-001 2026-03-21). Files in git checkout but NOT deployed to production `~/room-correction/` path on Pi. Also `measure_nearfield.py` still references pycamilladsp. Needs redeployment + code fix. |
...
- **US-059** (Phase: **TEST, DoD 14/14** — advanced 2026-03-21): GraphManager Core + Production Filter-Chain (Phase A). **DoD: 14/14 — ALL ITEMS SATISFIED.** QE confirmed TEST gate satisfied: 168 regression tests (S-1-S-4) + I-1 integration tests constitute the formal test plan; hardware stability (#8) and latency (#9) evidence already captured with owner acceptance. QE sign-off #13 unconditional. **Tracked follow-ups:** F-033 (Reaper JACK bridge RT), I-1 CI wiring, spectral verification (AC line 3141), D-042 lifting (q256 stability). GM tasks: 14/15 (GM-8 CamillaDSP removal tracked separately). HEAD: `5fbd5f7`. **Next phase: DEPLOY** (L-022: TEST -> **DEPLOY** -> VERIFY -> REVIEW). DEPLOY gate: "QE test plan executed, all criteria pass."
...
- **D-002 DEPLOY session** (**CLOSED** — 2026-03-21): US-052/US-060/US-061 files + binaries deployed to Pi. **VERIFY results (S-001):** US-052 PASS (with F-034/F-035 workarounds), US-060 BLOCKED (production path gap), US-061 BLOCKED (production path gap + pycamilladsp reference). US-062 regression PASS. HEAD: `afc0446`.
- **US-052** (Phase: **VERIFY, DoD 4/6** — S-001 2026-03-21): RT Signal Generator. VERIFY PASS with workarounds. Signal-gen running on Pi, responds to RPC. F-034 (clap negative value, workaround: `=` syntax) and F-035 (seccomp SIGSYS, workaround: relaxed profile) both HIGH — repo service file needs fixes. SG-13 deferred (graceful startup). US-053 unblocked.
- **US-060** (Phase: **DEPLOY, DoD 1/7** — BLOCKED 2026-03-21): PipeWire Monitoring Replacement (Phase B). Files in git checkout but NOT deployed to production `~/web-ui/` path. Production still has old CamillaDSP collectors. Needs redeployment to production path, then re-VERIFY. Blocks US-050 and US-051.
- **US-061** (Phase: **DEPLOY, DoD 0/8** — BLOCKED 2026-03-21): Measurement Pipeline Adaptation (Phase C). Files in git checkout but NOT deployed to production `~/room-correction/` path. `measure_nearfield.py` still references pycamilladsp. Needs redeployment + code fix, then re-VERIFY.
- **US-050** (ACTIVE — blocked on US-060): Measurement Pipeline Mock Backend. CI tier done, E2E tier needs D-040 adaptation. Blocked until US-060 VERIFY passes.
- **US-051** (ACTIVE — blocked on US-060): Persistent System Status Bar. UI structure done (SB-1-7), data source wiring blocked until US-060 VERIFY passes.
- **US-053** (ACTIVE — unblocked by US-052 VERIFY PASS): Manual Test Tool Page. TT-2 + PCM-MODE-3 code preserved. Was IMPLEMENT 3/6. Can now proceed.
...
| F-034 | High | Open | US-052: clap rejects negative `--max-level-dbfs` in service file. Workaround: `=` syntax on Pi. |
| F-035 | High | Open | US-052: seccomp filter kills PipeWire client init (SIGSYS). Workaround: relaxed profile on Pi. Same issue as GM service. |
