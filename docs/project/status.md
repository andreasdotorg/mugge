...
| US-050 | REVIEW | 4/6 | **in-review — owner NOT ACCEPTED, scope revision.** CI-tier DONE (4/4): mock backend, e2e scenario, architect APPROVED, QE APPROVED, AE APPROVED. **Owner requires E2E-tier:** real PipeWire + CamillaDSP + signal-gen + D-036 + web UI + pcm-bridge; room simulator = only mock. (5) E2E harness: NOT STARTED — architect designing PipeWire filter-chain approach. (6) E2E Playwright suite: NOT STARTED. |
...
| US-052 | IMPLEMENT | 1/6 | 9/12 subtasks done (SG-1 `ecad1d6`, SG-2 `981b824`, SG-3 `a10bd95`, SG-4 `5bfab1b`, SG-5 `ba804eb`, SG-6 `40dbb41`, SG-7 `3b155d8`, SG-8 `c637574`, SG-9 `9cd316f`). **All local Rust code complete (6,183 lines, 8 modules).** SG-10 in progress (local done, Cargo.lock Pi blocker — CM handling). SG-11/12 blocked. (1) Design doc: DONE. (2) Binary on Pi: not yet. (3) Integration test: not yet. (4) RT safety audit: not yet. (5) 5-min stability: not yet. (6) Lab note: not yet. |
...
- **TK-151** (in-progress, **AD-F006 BUILD CHAIN VALIDATED**): pcm-bridge binary built on Pi (2.6MB ARM aarch64 ELF, 110 crates). S-001 build #7 SUCCESS after 7 attempts. Type fix `77e7710`. **AD-F006 GATE PASSED:** Rust-on-Pi build chain proven — compilation, pipewire-rs linking, aarch64 PREEMPT_RT all work. **Steps 3-10 (runtime validation) IN PROGRESS** — worker-pcm-bridge running. **Unblocks:** SG-10, SB-7b, all Pi-gated subtasks.
...
- **US-052** (Phase: **IMPLEMENT** — 9/12 subtasks done, **all local Rust code complete, AD-F006 build chain VALIDATED**): Real-time signal generator in Rust (6,183 lines, 8 modules). D-037 APPROVED (TK-234 DONE). SG-1 through SG-9 ALL DONE. **SG-10 IN PROGRESS:** Cargo.lock resolved (`f56f0f5`), nix build compiling on Pi. SG-11/12 blocked on SG-10. Critical path: SG-10->SG-11->SG-12. TK-151 runtime validation in progress. Supersedes TK-229. Blocks US-053, US-047, US-012.
- **US-050** (Phase: **REVIEW** — owner NOT ACCEPTED, scope revision required): Measurement pipeline test harness. **CI-tier DONE:** MockSoundDevice + MockCamillaClient committed (`6cf1d12`, `3b73bb3`, `1bf8ed4`). 15/15 e2e + 282/282 regression PASS. All 3 advisors APPROVED CI-tier. **E2E-tier NOT STARTED:** Owner requires full stack E2E harness — real PipeWire, CamillaDSP, signal-gen, D-036 daemon, web UI, pcm-bridge all running; room simulator (PipeWire filter-chain) = only mock. Architect designing PipeWire integration approach. New tasks needed once design lands. Blocks US-047.
