...
| US-052 | IMPLEMENT | 3/6 | 11/12 subtasks done. **SG-12: TK-236 VERIFIED. BUG-SG12-7 CODE-COMPLETE — missing `node.group` property. Build #9 pending.** 7 bugs found total: SG12-1/3/5/6 FIXED, SG12-2 MEDIUM (RPC latency), SG12-4 Fix B code-complete, SG12-7 HIGH code-complete (native PW streams need `node.group` to join driver scheduling group). T1 FAIL (42ms), T2-T7 pending retest after build #9. (1) Design doc: DONE. (2) Binary on Pi: DONE. (3) Integration test: DONE. (4) RT safety audit: PASS. (5) Stability: pending retest. (6) Lab note: not yet. |
...
- **US-052** (Phase: **IMPLEMENT** — BUG-SG12-7 CODE-COMPLETE, build #9 pending): Real-time signal generator in Rust (6,183 lines, 8 modules). D-037 APPROVED. SG-1 through SG-11 ALL COMMITTED. **SG-12: 7 bugs found.** TK-236 VERIFIED (8 AUX ports linked, build #8). BUG-SG12-7 root cause: native PW streams need `node.group` property to join driver scheduling group — JACK clients get this automatically, native streams do not. Fix: `"node.group" => "group.dsp.0"` on both streams (2 lines). **Bug totals:** SG12-1/3/5/6 FIXED (committed), SG12-2 MEDIUM (RPC latency, architectural), SG12-4 Fix B code-complete, SG12-7 HIGH code-complete. **Next:** CM commit + build #9, verify streams reach Streaming + retest T2-T7. Commits: `e74ad99`, `dcbcf4d`, `9c79856`, `537b56a`, `c3cf92a`. Unblocks US-053, US-047, US-012 (when verified).
