...
| US-050 | DEFERRED | 5/6 | **deferred** (owner directive 2026-03-16: deselected for Tier 11. Was IMPLEMENT 5/6 — E2E code complete, pending Pi execution. Work preserved.) |
...
| US-059 | IMPLEMENT | 3/14 | **in-progress** (Phase A). **DoD: 3/14** (#1 GM-0 PASS, #2 filter-chain on Pi, #6 statically validated). #8 Mixxx PASS (11h+44m soak), Reaper pending. **All 4 advisory gates responded:** #10 Architect **APPROVED**, #11 Security CONDITIONAL (F-032 MUST-FIX), #12 AD findings partially resolved (TK-249 Mult verified by owner — downgraded to calibration investigation; 1 MUST-INVESTIGATE remains: convolver node.group at q256), #13 QE **BLOCKED** on G-1 (Reaper soak q256), G-2 (latency measurement), G-3 (integration regression tests). TK-238+239 committed (`984dac6` + `d99badd`). GM tasks: 11/15 done. HEAD: `d99badd`. **Next:** F-032 (P1), latency measurement (P2), Reaper soak+latency at q256 (P3, unblocks QE). |
...
- **US-059** (Phase: **IMPLEMENT, DoD 3/14** — owner-authorized 2026-03-16, D-040): GraphManager Core + Production Filter-Chain (Phase A). **GM-12 DJ stability: 11h+44m soak ZERO xruns.** DoD: #1 GM-0 gate PASS, #2 filter-chain on Pi, #6 statically validated. #8 Mixxx PASS, Reaper pending. **Advisory gates:** #11 Security CONDITIONAL (F-032/SEC-GM-01 MUST-FIX), #12 AD 2 BLOCKING (TK-249 safety-critical). #10 Architect + #13 QE pending. GM tasks: 11/15 done. **TK-238+239 ready for commit** (architect APPROVED, AE CONFIRMED). O-004 Pi health PASS (44 min, 0 ERR, 19% CPU). **SAFETY BLOCKER: TK-249** (linear Mult verification — blocks deployment). **Next:** TK-249 verification (highest priority), then F-032 fix, then Reaper stability test (DoD #8) + latency measurement (DoD #9).
...
- **US-052** (DEFERRED — owner directive 2026-03-16): Was IMPLEMENT 3/6. Code complete (6,183 lines, 193 tests). SG-12 ABORTED (architecture). Resumes after Tier 11 (US-059+).
- **US-050** (DEFERRED — owner directive 2026-03-16): Was IMPLEMENT 5/6. E2E code complete, pending Pi execution. Work preserved.
- **US-053** (DEFERRED — owner directive 2026-03-16): Was IMPLEMENT 3/6. TT-2 committed, PCM-MODE-3 code-complete. Work preserved.
...
- **TK-249** (**CRITICAL, safety blocker**): PW `linear` Mult parameter may be silently ignored — SPL doesn't match theory. **AD finding:** thermal safety depends on gain control working. If Mult is silently ignored (like config.gain), per-channel attenuation is ineffective and speakers are at risk (CHN-50P 7W thermal limit). **Blocks deployment.** Must be verified before any PA-on operation.
...
| TK-249 | **Critical** | Open | **SAFETY BLOCKER:** PW `linear` Mult may be silently ignored — thermal safety depends on gain control working. AD finding: if ignored like config.gain, speakers at risk. Blocks deployment. |
