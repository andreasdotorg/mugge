...
- **US-051** (code complete, pending Pi validation): Persistent system status bar with channel meters. SB-1 through SB-6 DONE, SB-4 DONE (reopened then completed). SB-7 Phase A PASS (12/12). SB-7 Phase B (live Pi testing) pending deployment.
- **US-052** (in-progress, architect decomposition complete): Real-time signal generator in Rust. D-037 APPROVED (TK-234 DONE). **SG-1 through SG-12 filed (2026-03-15).** 3 parallel tracks: Rust A (generators), Rust B (safety/RPC/capture), Python C (client). Critical path: SG-1->SG-4->SG-5->SG-6->SG-7->SG-8->SG-10->SG-11->SG-12. SG-1 through SG-5 + SG-9 are pure software (no Pi needed). TK-151 nix build in progress (gates Pi integration tests). Supersedes TK-229. Blocks US-053, US-047, US-012.
- **US-053** (selected, owner-authorized 2026-03-15; blocked): Manual test tool page. Blocked on US-051 + US-052. UX spec complete (`test-tool-page.md`).
