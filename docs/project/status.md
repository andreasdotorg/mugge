...
| User stories | active | 59 stories (US-000 through US-055 incl. US-000a, US-000b, US-011b, US-027a, US-027b) in `docs/project/user-stories.md`. Tier 5 (US-039-043): driver database. US-044: bypass protection. US-045-049: measurement safety + Path A + visualization (owner planning brief 2026-03-13). US-050: measurement mock backend / E2E test harness (owner directive 2026-03-14, scope expanded 2026-03-15). **Tier 9 (US-051-053): observable tooling (owner strategic pivot 2026-03-15).** US-051: persistent status bar. US-052: RT signal generator (Rust). US-053: manual test tool page. **Tier 10 (US-054-055): ADA8200 mic input for measurements (AE calibration transfer assessment 2026-03-15).** US-054: ADA8200 mic channel selection. US-055: calibration transfer from UMIK-1 to ADA8200 mic. US-011b and US-012 amended with power budget validation and automated gain calibration. US-047/048/049 implementation gated on UX design validation (TK-160 -> TK-161 -> TK-162). |
...
| US-050 | IMPLEMENT | 5/6 | **in-progress — E2E-tier code WRITTEN, not yet validated.** CI-tier DONE (4/4). E2E-tier: 8/8 tasks code-complete and committed. (5) E2E harness code: DONE. (6) E2E test suite: code committed, **NOT YET EXECUTED on Pi** (tests require PipeWire). Cannot advance to TEST until E2E tests pass on Pi. |
| US-051 | IMPLEMENT | 2/4 | IMPLEMENT in progress — SB-1-6 done, SB-7 Phase A 12/12 PASS. **Playwright E2E CODE-COMPLETE** (`test_status_bar.py`, 504 lines, 28 cases). Pending CM commit + Nix env execution. (1) Status bar visible: code done, **Playwright E2E written** (AC-1, 6 cases). (2) UX sign-off: pending — Playwright screenshots will be reviewed by UX specialist. (3) No regressions: **Playwright E2E written** (DoD-3, 4 cases). (4) 1280px viewport: **Playwright E2E written** (AC-10, 3 cases). **Next:** CM commit, run in Nix, UX screenshot review, same tests on Pi, owner validates perceptual items. |
| US-052 | IMPLEMENT | 3/6 | 11/12 subtasks done. **SG-12 CRITICAL: T2-T4 INVALID (false positives), T5 FAIL, T7 INVALID. BUG-SG12-5 CRITICAL — missing `media.class` + SPA format params.** T1 FAIL (42ms latency), **T2-T4 INVALID** (RPC ack only, no audio — no PW ports created), **T5 FAIL** (empty capture — BUG-SG12-4), T6 blocked, **T7 INVALID** (no audio flowing). **BUG-SG12-5 (CRITICAL, Architect diagnosed):** Missing `media.class` property + empty SPA format params in `stream.connect()`. Fix: ~100 lines in `main.rs`. Fix spec sent to team-lead. BUG-SG12-1/2/3 FIXED, BUG-SG12-4 Fix B in progress. (1) Design doc: DONE. (2) Binary on Pi: DONE. (3) Integration test: DONE. (4) RT safety audit: PASS. (5) Stability: **BLOCKED by BUG-SG12-5**. (6) Lab note: not yet. |
| US-053 | IMPLEMENT | 3/6 | TT-1 scaffold + **TT-2 COMMITTED (`ac5cfce`). PCM-MODE-3 CODE-COMPLETE** (capture spectrum + 13 Playwright E2E). Frontend: `test.js` now 836 lines (+514 spectrum: FFT pipeline, WS auto-reconnect, mountain range, source selector, tab lifecycle). Backend: `routes.py` (317 lines) + PCM-MODE-2 `/ws/pcm/{source}` + discovery API. **Playwright E2E: `test_capture_spectrum.py` (13 tests)** -- canvas, source selector, WS, switching, lifecycle, REST. PCM-TEST-2: 33 pytest tests for `/ws/pcm/{source}` (in `nix flake check`). Remaining: SPL, UMIK-1 hot-plug. (1) Page implemented: **COMMITTED + spectrum CODE-COMPLETE**, pending CM commit + Pi validation. (2) Signal-gen integration: committed (via SG-11). (3) Integration test: **Playwright E2E written** (13 capture + TT-2 117 unit). (4) Hot-plug test: not yet. (5) AD sign-off: not yet. (6) AE sign-off: not yet. |
...
- **TK-151** (**DONE — S-001 Steps 3-9 ALL PASS**): pcm-bridge runtime validated on Pi. AD-F006 build chain + runtime both proven. **TK-236 OPEN (LOW)** — architect investigation complete: 1-port behavior is correct by design (`pw_stream` delivers multi-channel interleaved). Option A (scripted `pw-link`) recommended as pragmatic solution. Needs Pi validation. Non-blocking. Step 10 (extended stability) deferred.
- **US-051** (Phase: **IMPLEMENT** — Playwright E2E needed before Pi/owner validation): Persistent system status bar with channel meters. SB-1 through SB-6 DONE. SB-7 Phase A PASS (12/12). **Owner process correction:** (1) Playwright E2E against mock first (DOM checks, tab presence, meter structure, canvas, colors, responsive, ABORT, WS), (2) UX reviews screenshots, (3) E2E on Pi with real data, (4) owner validates human-perceptual items only. **Playwright E2E is AC for US-051.** TP-003 manual checklist is supplement for hardware-only checks. All code committed. QE test plan TP-003 delivered (`6a75940`, 37 criteria). **Next step: worker writes Playwright E2E tests for TP-003 automatable criteria.**
- **US-052** (Phase: **IMPLEMENT** — **BLOCKED by BUG-SG12-5 CRITICAL, fix spec ready**): Real-time signal generator in Rust (6,183 lines, 8 modules). D-037 APPROVED. SG-1 through SG-11 ALL COMMITTED. **SG-12 Pi acceptance CRITICAL FAILURE:** T1 FAIL (42ms), **T2-T4 INVALID** (false positives — RPC ack only, no PW ports created), T5 FAIL (BUG-SG12-4), T7 INVALID. **BUG-SG12-5 (CRITICAL, Architect diagnosed):** Missing `media.class` property + empty SPA format params in signal-gen's `stream.connect()`. PipeWire never creates ports, so nothing can be linked. **Fix: ~100-line change in `main.rs`** — add `media.class` + SPA format pods to both always-on streams. No WirePlumber rules or API changes needed. Architect fix spec sent to team-lead for worker dispatch. NOT same root cause as TK-236 (PM earlier analysis corrected). BUG-SG12-1/2/3 FIXED (committed). BUG-SG12-4 Fix B in progress. Commits: `e74ad99`, `dcbcf4d`, `9c79856`. Unblocks US-053, US-047, US-012 (when resolved).
- **US-050** (Phase: **IMPLEMENT** — E2E-tier code committed, NOT YET VALIDATED on Pi): Measurement pipeline test harness. **CI-tier DONE (4/4 DoD).** E2E-tier: 8/8 tasks code-complete and committed (EH-1 `1344415`, EH-2 `a1f9ad1`, EH-3 `36fc614`, EH-4 `5c2be5e`, EH-5 `35e28f5`, EH-6 `f642931`+`e440e3c`, EH-7 `b58b1e6`, EH-8 `8b6bc1f`). **Tests NOT YET EXECUTED on Pi** (require PipeWire). Cannot advance to TEST until E2E tests pass on Pi. Blocks US-047.
- **US-053** (Phase: **IMPLEMENT** — TT-2 COMMITTED + PCM-MODE-3 CODE-COMPLETE, pending CM commit + Pi validation): Manual test tool page. TT-1 scaffold DONE (`f3fcfa2`). **TT-2 DONE** (`ac5cfce`). **PCM-MODE-3 CODE-COMPLETE:** capture spectrum analyzer (+514 lines in `test.js`: FFT pipeline, WS auto-reconnect, mountain range rendering, source selector via `/api/v1/pcm-sources`, tab lifecycle) + 13 Playwright E2E tests (`test_capture_spectrum.py`). PCM-TEST-2: 33 pytest tests for `/ws/pcm/{source}` endpoint (D-024 compliant, in `nix flake check`). Remaining items: SPL computation, UMIK-1 hot-plug.
