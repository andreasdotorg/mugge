...
| User stories | active | 63 stories (US-000 through US-059 incl. US-000a, US-000b, US-011b, US-027a, US-027b) in `docs/project/user-stories.md`. Tier 5 (US-039-043): driver database. US-044: bypass protection. US-045-049: measurement safety + Path A + visualization (owner planning brief 2026-03-13). US-050: measurement mock backend / E2E test harness (owner directive 2026-03-14, scope expanded 2026-03-15). **Tier 9 (US-051-053): observable tooling (owner strategic pivot 2026-03-15).** US-051: persistent status bar. US-052: RT signal generator (Rust). US-053: manual test tool page. **Tier 10 (US-054-055): ADA8200 mic input for measurements (AE calibration transfer assessment 2026-03-15).** US-054: ADA8200 mic channel selection. US-055: calibration transfer from UMIK-1 to ADA8200 mic. **Tier 11 (US-056-059): architecture evolution (owner directive 2026-03-16).** US-056: CamillaDSP JACK backend (Phase 0). US-057: PW-native backend investigation (spike). US-058: PW filter-chain FIR benchmark (BM-2). US-059: GraphManager sole PipeWire session manager (D-039: owner approved 2026-03-16, draft). US-011b and US-012 amended with power budget validation and automated gain calibration. US-047/048/049 implementation gated on UX design validation (TK-160 -> TK-161 -> TK-162). |
...
| US-052 | IMPLEMENT | 3/6 | 11/12 subtasks done. **SG-12 ABORTED BY OWNER (2026-03-16).** Owner decision: fighting ALSA Loopback architecture for native PW stream activation is not viable. Architecture evolution (US-056 JACK backend) supersedes. 7 bugs found: SG12-1/3/5/6 FIXED, SG12-2/4/7 WONT-FIX (architectural). T1 FAIL, T2-T7 never tested. Signal-gen Pi validation BLOCKED BY ARCHITECTURE, not by code. Code is complete (6,183 lines, 193 tests pass). (1) Design doc: DONE. (2) Binary on Pi: DONE. (3) Integration test: DONE. (4) RT safety audit: PASS. (5) Stability: BLOCKED (architecture). (6) Lab note: not yet. |
...
- **US-052** (Phase: **IMPLEMENT** — SG-12 ABORTED, blocked by architecture): Real-time signal generator in Rust (6,183 lines, 8 modules). D-037 APPROVED. SG-1 through SG-11 ALL COMMITTED. **SG-12 ABORTED BY OWNER (2026-03-16).** Owner decision: fighting ALSA Loopback architecture for native PW stream activation (missing `media.class`, SPA format negotiation, `audio.position`, `node.group` — all properties JACK clients get automatically) is not viable. 7 bugs found in acceptance testing; the pattern of bugs proves the architecture is wrong, not the code. **Architecture evolution (US-056 JACK backend) supersedes.** Under JACK backend, CamillaDSP has per-channel JACK ports — signal-gen and pcm-bridge link directly without SPA format negotiation. BUG-SG12-2/4/7 closed WONT-FIX (architectural). D-002 deploy session CLOSED. **Code preserved:** 193 tests pass, binary builds on Pi. Pi validation resumes after US-056. Commits: `e74ad99`, `dcbcf4d`, `9c79856`, `537b56a`, `c3cf92a`.
