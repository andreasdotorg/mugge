...
class SignalGenClient:
    ...
    def playrec(self, output_buffer, samplerate=48000, input_mapping=None,
        ...
        # Use sweep if the signal has a frequency sweep characteristic,
        # otherwise use the raw signal approach via playrec.
        # For sd-compatibility, we send the signal content as a playrec
        # command with the appropriate signal type.
        # Since the real signal is generated by the Rust binary, we just
        # need to describe what to play.  For sd-compatible callers that
        # build their own waveforms, we detect the signal type heuristically.
        cmd = {
            "cmd": "playrec",
            "signal": "pink",  # default; sd callers typically play pink noise
